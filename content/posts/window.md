...
`CUME_DIST()`는 현재 행보다 크거나 같은 값(ORDER BY DESC 기준)의 누적 비율을 반환한다.

계산식:

현재 행까지의 누적 건수 / 전체 행 수

예를 들어 급여가 5000인 이영희는 상위 2명(6000, 5000)에 포함되므로:

```text
2 / 4 = 0.5
```

즉 전체 데이터 중 상위 50% 위치에 있다는 의미이다.
...
- `(개발, NULL)` : 개발 부서 전체 평균 급여 (소계)
- `(영업, NULL)` : 영업 부서 전체 평균 급여 (소계)
- `(NULL, NULL)` : 전체 직원 평균 급여 (총계)

즉 `ROLLUP(부서, 직무)`는 다음 순서로 집계한다.
...
따라서 상세 데이터 → 부서별 소계 → 전체 총계가 순차적으로 생성된다.
---
...
즉 `CUBE(부서, 직무)`는 다음과 같은 집계를 모두 생성한다.
...
CUBE는 ROLLUP과 달리 **직무별 집계 결과까지 추가로 생성**하므로 더 많은 결과를 반환한다.
...
`GROUPING SETS`는 필요한 집계만 직접 지정할 수 있어 불필요한 소계나 총계를 생성하지 않는다.
