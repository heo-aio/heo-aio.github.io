...
⚠️ 여기서 좀 헤맨 부분이 있었는데, `shap_values`가 라이브러리 버전에 따라 배열 하나로 나오기도 하고 클래스별 리스트로 나오기도 한다. 예전 튜토리얼 코드 그대로 따라 하면 `sv[:, :, 1]`이 필요한지 그냥 `sv`를 쓰면 되는지 버전마다 달라서 한참 검색했다. 지금 버전 기준으로는 위처럼 `shap.Explainer`로 한 번 감싸서 쓰는 API가 정석인 것 같다.
