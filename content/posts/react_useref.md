...
1. state 스냅샷 문제가 `setTimeout` 외에 Promise, API 응답 등 다른 비동기 콜백에서도 똑같이 재현되는지 확인해보기
2. ref 값을 화면에 그대로 표시하려고 하면 왜 실시간으로 안 보이는지 직접 재현해서 버그 사례로 남겨보기
3. `let`과 `const`로 선언한 ref 변수의 동작 차이가 실제로 있는지 확인해보기
