...
여기까지가 헷갈리기 쉬워서, 앙상블의 핵심 개념을 그림 한 장으로 정리했다.

![앙상블 개념도](/images/ml/건강분류_앙상블_개념도.png)

## 출발점: 모델은 '정답'이 아니라 '확률'을 뱉는다

각 모델은 한 학생에 대해 클래스별 확률 벡터를 내놓는다. 이 중 제일 큰 값(`argmax`)이 그 모델의 예측이다. **앙상블은 이 확률들을 어떻게 합치느냐의 문제다.**

한 학생(정답 = `unhealthy`)에 대해 네 모델이 이렇게 예측했다고 해보자.

| 모델 | fit | at-risk | unhealthy | 각자 예측 |
|---|---|---|---|---|
| LightGBM (약) | 0.10 | **0.60** | 0.30 | at-risk (오답) |
| XGBoost (약) | 0.10 | **0.58** | 0.32 | at-risk (오답) |
| CatBoost (강) | 0.05 | 0.35 | **0.60** | unhealthy (정답) |
| HistGB (강) | 0.05 | 0.38 | **0.57** | unhealthy (정답) |

## ① 단순 평균 — 1인 1표

4개 확률을 그냥 평균낸다(`np.mean`).

```text
at-risk   = (0.60 + 0.58 + 0.35 + 0.38) / 4 = 0.478
unhealthy = (0.30 + 0.32 + 0.60 + 0.57) / 4 = 0.448
→ argmax = at-risk (오답)
```

강한 두 모델은 맞혔는데, 약한 두 모델이 at-risk로 표를 몰아서 평균이 오답으로 끌려갔다. 앞에서 단순 평균 앙상블이 최고 단일 모델보다 낮게 나온 게 바로 이 구조 때문이다.

## ② 가중 평균 — 잘하는 모델에 무게

모델마다 가중치를 다르게 준다. 앞서 찾은 실제 가중치(CatBoost 0.53, HistGB 0.44, 나머지 3%)를 적용하면,

```text
at-risk   = 0.01·0.60 + 0.02·0.58 + 0.53·0.35 + 0.44·0.38 = 0.370
unhealthy = 0.01·0.30 + 0.02·0.32 + 0.53·0.60 + 0.44·0.57 = 0.578
→ argmax = unhealthy (정답)
```

같은 4개 예측인데, 약한 둘을 거의 무시하니 정답으로 돌아왔다. "합이 1인 랜덤 가중치를 수천 번 뽑아 OOF 점수가 가장 높은 조합을 채택"한 게 Dirichlet 랜덤서치다.

## ③ 스태킹 — 합치는 규칙을 또 다른 모델이 학습

가중 평균이 "모델당 무게 하나"를 사람이 탐색해 정한 것이라면, 스태킹은 그 합치는 규칙 자체를 **메타 모델(로지스틱 회귀)** 에게 학습시킨다. 4개 모델의 확률(4×3=12개)을 입력 특징으로 넣어, "CatBoost가 unhealthy라고 할 땐 특히 믿고…" 같은 **모델×클래스 단위의 조합 규칙**을 데이터로 배운다. (스태킹은 [4편](../student_health_04_stacking/)에서 자세히 다룬다.)

> 그리고 사실 이 셋 이전에 이미 앙상블을 한 번 했다. 각 모델이 test를 예측할 때 5개 fold 모델의 예측을 평균낸 것(OOF의 fold 평균)이 그것이다. 같은 모델이라도 fold마다 조금씩 다르게 학습되니, 평균내서 우연에 덜 흔들리게 만든 셈이다.
